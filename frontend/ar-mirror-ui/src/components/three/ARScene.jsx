...
export default function ARScene({ posePoints, garment, streamUrl }) {
  ...
  useThreeScene(() => {
    ...
    const liveStream = document.createElement('img');
    liveStream.className = styles.stream;
    liveStream.alt = 'Live camera stream';
    liveStream.src = streamUrl || '';
    if (streamUrl) {
      mount.appendChild(liveStream);
    }
    ...
    return () => {
      ...
      if (liveStream.parentElement === mount) {
        mount.removeChild(liveStream);
      }
      renderer.dispose();
